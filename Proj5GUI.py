...
#		self.repaint()
    ...
    def paintEvent(self, event):
        ...
        align = QTextOption(Qt.AlignmentFlag.AlignHCenter |
...
class Proj5GUI(QMainWindow):
    ...
    def generateNetwork(self):
        ...
        self.genParams = {'size': self.size.text(
        ), 'seed': self.curSeed.text(), 'diff': diff}
    ...
    def generateClicked(self):
        ...
        self.view.addPoints([QPointF(c._x, c._y)
                            for c in self._scenario.getCities()], (0, 0, 0))
    ...
    # what about calling this somehow every time a new bssf is found?
    def displaySolution(self):
        # get rid of edge labels but not point labels
        self.view.clearEdges([(64, 64, 255)])
        if self._solution:
            self.addCities()
    ...
    # need to reset display??? and say "processing..." at bottom???
    def solveClicked(self):
        ...
        solve_func = 'self.solver.' + \
            self.ALGORITHMS[self.algDropDown.currentIndex()][1]
...
#		app.processEvents()
    ...
    def initUI(self):
        ...
        # Weird hack to make box wide enough to show all of last item
